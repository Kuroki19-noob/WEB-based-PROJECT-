...
const createAccount = async(req,res) => {
    try {
        ...
        const data = await db.query(`INSERT INTO accounttbl (username, password, email) VALUES (?,?,?)`,[username, hashpass, email])
        if (!data){
            return res.status(500).send({
                success:false,
...
const updateAccount = async (req, res) => {
    try {
        ...
        const values = [username, email, accID]

        let query = `UPDATE accounttbl SET username=?, email=?`

        if (password) {
            const hashpass = await bcrypt.hash(password, 10)
            query = `UPDATE accounttbl SET username=?, password=?, email=?`
            values.splice(1, 0, hashpass)
        }

        const data = await db.query(`${query} WHERE accountID = ?`, values)
